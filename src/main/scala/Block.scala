...
case class Block(center: (Double,Double,Double), val faces: List[Face]) {
  ...
  def intersects(discontinuity: Discontinuity) = {
    ...
    // Restrict our attention to plane of discontinuity
    solver.addConstraint(Array[Double](discontinuity.a, discontinuity.b, discontinuity.c, 0.0),
                         LpSolve.EQ, discontinuity.d)
    // Require s to be within planes defined by faces of block
    faces.foreach {face => solver.addConstraint(Array[Double](face.a, face.b, face.c, -1.0), LpSolve.LE, face.d)}
    // Require s to be within planes defining shape of discontinuity
    discontinuity.globalCoordinates.foreach {case ((a,b,c),d) =>
