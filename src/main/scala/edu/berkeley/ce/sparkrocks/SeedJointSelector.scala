...
object SeedJointSelector {
  ...
  /**
    ...
    * @param seedJoints Seq of Joints that have been identified as seed joints
    * @param nJointSets Number of joint sets in the initial input joint set
  ...
  def searchJointSets(jointSets: Seq[Seq[Joint]], inputVolume: Block, numProcessors: Int,
                      ...
                      seedJoints: Seq[Joint] = Seq.empty[Joint], nJointSets: Int): (Seq[Joint], Seq[Joint]) = {
    ...
    if (jointSets.head(0).shape.isEmpty) {
      ...
      // Not enough seed joints found
        ...
        // TODO: NEED TO COUNT NUMBER OF PARTITIONS THAT HAVE BEEN GENERATED BY CURRENT SEED JOINTS AND ADJUST NUMPROCESSORS ACCORDINGLY!!!
        searchJointSets(jointSets.tail, inputVolume, numProcessors - newSeedJoints.length,
          remainingJoints ++ remainingFromJointSet, newSeedJoints, nJointSets)
  ...
  private def findSeedJoints(jointSet: Seq[Joint], rockVolume: Block, numProcessors: Integer, tolerance: Double = 0.01,
    ...
    // TODO: Revise logic here - maybe this stepsize thing is unnecessary or should only be triggered once all joint sets have been traversed once?
    if (stepSize < 0.01) {
  ...
  private def selectSeedJoints(jointSet: Seq[Joint], initialBlock: Block, volumePerProcessor: Double,
    ...
    // TODO: Review logic here - it is better to have more rather than less seed joints
